const ButtonUi = ({ title }: { title: string }) => {
  return (
    <div className="flex lg:flex-row flex-col lg:space-x-[16px] space-y-4 lg:space-y-0 w-full lg:mb-0 mb-md max-w-xl">
      <a
        className="group md:py-[15.5px] py-[14px] px-[42px] rounded-xs shadow-button-primary text-link text-white flex items-center justify-center space-x-[8px] bg-[#fe8b53] font-semibold rounded-[3px] transition-all duration-300 transform hover:bg-[#f06b3c] "
        href="#"
      >
        <span className="">{title}</span>{" "}
        <svg
          className="group-hover:translate-x-1 transition-transform duration-300 hover:scale-105"
          width="6"
          height="9"
          viewBox="0 0 6 9"
          fill="none"
